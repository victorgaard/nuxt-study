...
<template>
    <PopoverRoot :open="isCartOpen" :onUpdate:open="(open) => isCartOpen = open">
        ...
        <PopoverPortal>
            <PopoverContent align="end" :side-offset="8"
                ...
                <div v-else class="flex flex-col min-w-[460px]">
                    <div class="overflow-y-auto overflow-x-hidden max-h-[calc(100dvh-280px)] -mx-4 px-4">
                        <div v-for="item in cart" :key="item.id" class="flex flex-col gap-4 pb-4">
                                ...
                                <div class="flex flex-col gap-0.5 flex-wrap">
                                    ...
                                    <div
                                        ...
                                        <button :disabled="item.quantity <= 1"
                                            ...
                                            @:click="cartStore.decrementProductQuantity(item.id)">-</button>
                                        <p>{{ item.quantity }}</p>
                                        <button
                                            class="px-3.5 hover:bg-slate-600 h-full bg-slate-700 disabled:bg-transparent disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                                            @:click="cartStore.incrementProductQuantity(item.id, 1)">+</button>
                                    </div>
                    ...
                    <button
                        class="flex justify-between mt-4 py-3 px-6 bg-emerald-600 rounded"><span>Checkout</span><span>Total:
