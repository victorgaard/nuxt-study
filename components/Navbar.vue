...
<template>
    <nav class="flex justify-between p-8 gap-4 text-lg font-medium">
        <div class="flex items-center gap-3">
            ...
            <div v-if="route && currentProductStore.currentProduct.name" class="hidden sm:flex items-center gap-3">
                <span class="text-slate-600">></span>
                <NuxtLink alt="Product page" :to="`/products/${route}`" class="line-clamp-1"
                    activeClass="text-emerald-400"> {{ currentProductStore.currentProduct.name }}</NuxtLink>
            </div>
        </div>
        <Cart />
    </nav>
